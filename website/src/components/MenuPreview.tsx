      ...
      <div className="container mx-auto px-6">
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-serif font-bold text-charcoal-950 mb-6">
            Our <span className="text-maroon-600 italic">Menus</span>
          </h2>
              ...
              <div className="grid md:grid-cols-2 gap-6">
                {category.menus.map((menu) => (
                  <Link
                    key={menu.id}
                    href={`/menus#${menu.id}`}
                    className="border-b border-maroon-100/60 pb-6 group block"
                  >
                    ...
                    <p className="text-charcoal-600 text-sm leading-relaxed">
                      {menu.items.slice(0, 3).join(" &middot; ")}
                      {menu.items.length > 3 ? " ..." : ""}
                    </p>
                  </Link>
                ))}
        ...
        <div className="text-center mt-14">
          <Link
            href="/menus"
            className="inline-flex items-center gap-2 px-8 py-4 bg-maroon-600 hover:bg-maroon-500 text-white font-medium rounded-sm transition-all shadow-md hover:shadow-maroon-600/20 group"
          >
